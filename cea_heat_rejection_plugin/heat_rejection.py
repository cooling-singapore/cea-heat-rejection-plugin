...
def get_building_groups(locator):
    ...
    district_cooling_systems = ['SUPPLY_COOLING_AS3', 'SUPPLY_COOLING_AS4']

    try:
        with open(locator.get_groups()):
            print("Using building groups generated by the user")

    # Creating building group files (if not provided by the user)
    except IOError:
    ...
    for i, row in building_groups.iterrows():
        ...
        for building in building_list:
            building_supply_system = supply_systems.loc[supply_systems.Name == building]
        ...
        if all(x == supply_system[0] for x in supply_system):
            if supply_system[0] in cooling_tower_systems:
                data_ct['Group'].append(row.Group)
                data_ct['Buildings'].append(row.Buildings)
            else:
                data_no_ct['Group'].append(row.Group)
                data_no_ct['Buildings'].append(row.Buildings)
        else:
